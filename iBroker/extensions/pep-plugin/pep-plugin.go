...
// declarative config
type Config struct {
	// type of the authorization endpoint, e.g. Keycloak
	AuthorizationEndpointType string
	// address of the authorzation endpoint, f.e. http://keycloak.org/
...
func (k Kong) Exit(code int, msq string) {
	messageBytes := []byte(msq)
	k.pdk.Response.Exit(code, messageBytes, make(map[string][]string))
}
...
func handleRequest(kong KongI, conf *Config) {
	...
	if conf.AuthorizationEndpointType == "Keycloak" {
